...
RUN git clone -b ros2 https://github.com/xsenssupport/Xsens_MTi_ROS_Driver_and_Ntrip_Client.git src/xsens
...
COPY $SRC_DIRECTORY/descriptions/alliander_xsens_description/ /$WORKDIR/ros/src/alliander_xsens_description/
RUN --mount=type=cache,id=apt-cache,target=/var/cache/apt,sharing=locked --mount=type=cache,id=apt-lists,target=/var/lib/apt,sharing=locked /$WORKDIR/rosdep_install.sh --build
...
RUN --mount=type=cache,id=uv-cache,target=/root/.cache/uv uv sync --group alliander-xsens \ 
